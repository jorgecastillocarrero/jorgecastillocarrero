"""
FMP - Descarga de Earnings (EPS Actual vs Estimado)
"""
import asyncio
import aiohttp
import psycopg2
...
class FMPEarningsDownloader:
    ...
    async def download_earnings(self, symbols):
        """Descargar earnings historicos"""
        print(f"\n[EARNINGS] Descargando earnings para {len(symbols):,} simbolos...")
    ...
    async def run(self):
        print("="*60)
        print("[START] FMP - DESCARGA DE EARNINGS")
        print("="*60)
...
if __name__ == "__main__":
    downloader = FMPEarningsDownloader()
