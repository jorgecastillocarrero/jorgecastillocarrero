"""
Descarga precios históricos de FMP usando el endpoint stable.
"""
import asyncio
import aiohttp
import psycopg2
    ...
    for p in prices:
        try:
            rows.append((
                ...
                float(p.get('low', 0)),
                float(p.get('close', 0)),
                float(p.get('close', 0)),  # adj_close = close
                int(p.get('volume', 0)),
...
async def main():
    ...
    semaphore = asyncio.Semaphore(5)  # Más conservador
    total_inserted = 0
    symbols_with_data = 0
...
if __name__ == '__main__':
    print('='*70, flush=True)
    print(f'DESCARGA PRECIOS HISTORICOS (STABLE API): {START_DATE} a {END_DATE}', flush=True)
    print('='*70, flush=True)
    asyncio.run(main())
    conn.close()
