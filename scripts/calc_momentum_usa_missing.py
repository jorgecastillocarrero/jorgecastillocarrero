"""
Calcula features de momentum para símbolos USA faltantes.
...
def get_missing_usa_symbols(conn):
    """Símbolos USA con datos pero sin momentum."""
    cur = conn.cursor()
    cur.execute("""
        SELECT DISTINCT p.symbol
        FROM fmp_profiles p
        WHERE p.country = 'US'
        AND EXISTS (SELECT 1 FROM fmp_income_statements i WHERE i.symbol = p.symbol)
        AND EXISTS (SELECT 1 FROM fmp_price_history ph WHERE ph.symbol = p.symbol)
        AND NOT EXISTS (SELECT 1 FROM features_momentum fm WHERE fm.symbol = p.symbol)
        ORDER BY p.symbol
    """)
...
def main():
    ...
    print(f"MOMENTUM USA FALTANTES - {datetime.now()}", flush=True)
    print(f"{'='*70}", flush=True)
    ...
    print("Obteniendo simbolos USA faltantes...", flush=True)
    symbols = get_missing_usa_symbols(conn)
    total = len(symbols)
    print(f"  {total:,} simbolos a procesar", flush=True)

    if total == 0:
        print("No hay simbolos pendientes.")
        conn.close()
        return
    ...
    for i, symbol in enumerate(symbols, 1):
        try:
            ...
            if len(df) < 60:  # Minimo para calcular volatilidad
                failed += 1
                continue
