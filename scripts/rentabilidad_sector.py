"""Rentabilidad por Sector - PERIODO 2026 (con tipos de cambio correctos)"""
import sys
sys.path.insert(0, '.')
...
print(f'DESGLOSE RENTABILIDAD POR ACCION - PERIODO 2026')
print(f'Fecha base: 31/12/2025 | Fecha fin: {latest}')
print()
print(f"{'Ticker':<10} {'Estado':<8} {'Base':<7} {'Mon':<4} {'P.Base':>10} {'P.Final':>10} {'Rent.%':>8} {'P&L EUR':>12} {'Sector':<25}")
print('-' * 110)

total_pnl = 0
for p in positions:
    sector_str = (p['sector'] or 'Sin datos')[:25]
    print(f"{p['ticker']:<10} {p['estado']:<8} {p['base_tipo']:<7} {p['currency']:<4} {p['p_base']:>10.2f} {p['p_final']:>10.2f} {p['rent_pct']:>+7.1f}% {p['pnl_eur']:>+12,.0f} {sector_str:<25}".replace(',', '.'))
    total_pnl += p['pnl_eur']
...
print(f"{'Sector':<25} {'Ops':>5} {'Rent.Media':>12} {'P&L EUR':>15}")
print('-' * 60)
for sector, lst in sorted_sectors:
    avg = sum(p['rent_pct'] for p in lst) / len(lst)
    pnl = sum(p['pnl_eur'] for p in lst)
    print(f"{sector:<25} {len(lst):>5} {avg:>+11.1f}% {pnl:>+15,.0f}".replace(',', '.'))
print('-' * 60)
print(f"{'TOTAL':<25} {len(positions):>5} {'':<12} {total_pnl:>+15,.0f}".replace(',', '.'))
