"""
Descarga precios históricos de FMP 2003-2008 usando el endpoint stable.
"""
import asyncio
import aiohttp
import psycopg2
...
def get_symbols_to_download():
    cur = conn.cursor()
    # Símbolos que ya tienen datos en 2009+ pero no en 2003-2008
    cur.execute('''
        ...
        AND LENGTH(symbol) <= 5
        AND symbol NOT IN (
            SELECT DISTINCT symbol FROM fmp_price_history WHERE date < '2009-01-01'
        )
        ORDER BY symbol
    ''')
